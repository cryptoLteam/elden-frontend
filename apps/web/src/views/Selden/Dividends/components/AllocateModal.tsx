...
const description = {
  allocate: {
    ...
    description: "Allocate sELDEN to this plugin to earn a share of our protocol's revenue",
  },
  deallocate: {
    title: 'Deallocate from dividends',
    description: "Deallocate sELDEN from this plugin to stop earning a share of our protocol's revenue",
  },
}
        ...
        <EBox>
          <Inner>
            <Flex flexDirection="column" flexGrow={1} justifyContent="center">
              ...
              />
              <Balance>
                BALANCE: {tab === 'allocate' ? `${data.sEldenBalance}sELDEN` : `${data.manualAllocation}Elden`}{' '}
              </Balance>
            </Flex>
            <ECircleButton onClick={() => setAmount(tab === 'allocate' ? data.sEldenBalance : data.manualAllocation)}>
              MAX
            </ECircleButton>
          </Inner>
        </EBox>
        ...
        {tab === 'deallocate' && amount && amount <= data.allocated ? (
          <Flex flexDirection="column" mt="12px" style={{ gap: '8px' }}>
            ...
            <Flex justifyContent="space-between">
              ...
              <Flex style={{ fontSize: '10px' }} alignItems="center">
                {(amount * dashboardData.deAllocationFee) / 100} sELDEN
              </Flex>
            </Flex>
          </Flex>
        ) : (
