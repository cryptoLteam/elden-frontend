...
const description = {
  allocate: {
    ...
    description: "Allocate sELDEN to this plugin to earn perks from our launchpad's partnering protocols",
  },
  deallocate: {
    title: 'Deallocate from launchpad',
    description: "Deallocate sELDEN from this plugin to stop earning from our launchpad's partnering protocols",
  },
}
        ...
        <EBox style={{ marginTop: '8px' }}>
          <Inner>
            <Flex flexDirection="column" flexGrow={1} justifyContent="center">
              ...
              />
              <Balance>
                BALANCE: {tab === 'allocate' ? `${userData.sEldenBalance} sELDEN` : `${userData.allocated} sELDEN`}{' '}
              </Balance>
            </Flex>
            <ECircleButton onClick={() => setAmount(tab === 'allocate' ? userData.sEldenBalance : userData.allocated)}>
              MAX
            </ECircleButton>
          </Inner>
        </EBox>
