...
import { CAKE, USDC, USDT, STABLE_COIN, ELDEN } from '@pancakeswap/tokens'
...
export function useDefaultsFromURLSearch():
  ...
  useEffect(() => {
    ...
    const parsed = queryParametersToSwapState(
      query,
      native.symbol,
      ELDEN[chainId]?.address ?? STABLE_COIN[chainId]?.address ?? USDC[chainId]?.address ?? USDT[chainId]?.address,
    )
