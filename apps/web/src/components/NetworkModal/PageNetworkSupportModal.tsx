  ...
  return (
    <Modal title={title || t('Check your network')} hideCloseButton headerBackground="gradientCardHeader">
      <Grid style={{ gap: '16px' }} maxWidth="360px">
        <Text bold>{t('It’s a Base only feature')}</Text>
        ...
        <Text small>
          {t(
            'Eldenfi features are currently available only on Base Chain! Come over and join the community in the fun!',
          )}
        </Text>
        {canSwitch ? (
          <Button
            ...
            isLoading={isLoading}
            onClick={() =>
              isWrongNetwork ? switchNetworkLocal(ChainId.BASE_TESTNET) : switchNetworkAsync(ChainId.BASE_TESTNET)
            }
          >
