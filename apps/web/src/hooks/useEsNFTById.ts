...
import { EsNFTData } from 'views/Positions/hooks/useEsNFTCardData'
...
export default function useEsNFTById(poolId: string | undefined, nftId: string | undefined, interval: number = 30000) {
  ...
  const {
    ...
    poolId && positionsSubgraphClient ? `useEsNFTListsById-${poolId}` : null,
    async () => {
