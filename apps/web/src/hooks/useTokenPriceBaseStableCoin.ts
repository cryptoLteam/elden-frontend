...
export function useTokenPriceBaseStableCoin(token1Address: string) {
  ...
  const token1AddressWrap =
    token1Address && token1Address.toLowerCase() === SELDEN_ADDRESS.toLocaleLowerCase() ? ELDEN_ADDRESS : token1Address
  const token0Address = STABLE_COIN[chainId ?? ChainId.BASE_TESTNET].address
